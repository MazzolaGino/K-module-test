...
abstract class AbstractController
{
    ...
    public function __construct(array $settings = [])
    {
        ...
        if (! isset($settings['access_token'], $settings['access_token_secret'], $settings['consumer_key'], $settings['consumer_secret'], $settings['bearer_token'])) {
    ...
    public function performRequest(string $method = 'GET', array $postData = []): mixed
    {
        try {
            ...
            if ($response->getStatusCode() >= 400) {
                ...
                throw new Exception(json_encode($error, JSON_THROW_ON_ERROR), $response->getStatusCode());
            }

            return $body;
        } catch (ClientException | ServerException $e) {
            throw new Exception(json_encode($e->getResponse()
                ->getBody()
                ->getContents(), JSON_THROW_ON_ERROR));
        }
    }
